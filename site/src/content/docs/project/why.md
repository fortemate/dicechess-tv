...
Dice Chess keeps the chess pieces and the way they move, and adds three dice. Each die names a piece: pawn, knight, bishop, rook, queen or king. A turn is up to three actions, one per die, with the pieces the dice allow, and a player must use as many dice as the position allows. There is no check and no checkmate: a game is won by taking the king.
...
The living room is where a family is already together. In a hotseat game two players pass one remote, turn by turn. A person alone can play the on-device bot. Nothing needs an account or a network.
